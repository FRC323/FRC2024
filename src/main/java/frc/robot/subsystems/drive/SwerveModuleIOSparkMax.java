...
public class SwerveModuleIOSparkMax implements SwerveModuleIO {
  ...
  public SwerveModuleIOSparkMax(
      int drivingCanId, int turningCanId, double moduleOffset, boolean isInverted) {
  ...
  public void updateInputs(SwerveModuleInputs inputs) {
    ...
    inputs.turnAngularVelocityRPM =
        Units.radiansPerSecondToRotationsPerMinute(turningEncoder.getVelocity());
  }

  @Override
  public void setCommandedOutputs(Rotation2d steeringAngle, double wheelVelocity) {
    drivingPIDController.setReference(wheelVelocity, CANSparkBase.ControlType.kVelocity);
    turningPIDController.setReference(
        steeringAngle.getRadians(), CANSparkBase.ControlType.kPosition);
  }

  @Override
  public void resetDriveEncoder(double newValue) {
    drivingEncoder.setPosition(newValue);
  }
  ...
  public boolean initModule() {
    return SparkMaxUtils.initWithRetry(this::initDriveSpark, Constants.SPARK_INIT_RETRY_ATTEMPTS) &&
    SparkMaxUtils.initWithRetry(this::initTurnSpark, Constants.SPARK_INIT_RETRY_ATTEMPTS);
  }
